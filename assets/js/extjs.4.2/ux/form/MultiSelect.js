 ...
 * @extends Ext.form.field.Base A control that allows selection and form
...
Ext.define('Ext.ux.form.MultiSelect', {
	...
	uses : ['Ext.view.BoundList', 'Ext.form.FieldSet',
			'Ext.ux.layout.component.form.MultiSelect', 'Ext.view.DragZone',
			'Ext.view.DropZone'],

	/**
	 * @cfg {String} listTitle An optional title to be displayed at the top of
	 *      the selection list.
	 */
	...
	/**
	 * @cfg {String} displayField Name of the desired display field in the
	 *      dataset (defaults to 'text').
	 */
	displayField : 'text',

	/**
	 * @cfg {String} valueField Name of the desired value field in the dataset
	 *      (defaults to the value of {@link #displayField}).
	 */
	 ...
	 *      no items (defaults to 'This field is required')
	 */
	...
	/**
	 * @cfg {String} delimiter The string used to delimit the selected values
	 *      when {@link #getSubmitValue submitting} the field as part of a form.
	 *      Defaults to ','. If you wish to have the selected values submitted
	 *      as separate parameters rather than a single delimited parameter, set
	 *      this to <tt>null</tt>.
	 */
	delimiter : ',',
	...
	bindStore : function(store, initial) {
		var me = this, oldStore = me.store, boundList = me.boundList;
	...
	onRender : function(ct, position) {
		var me = this, panel, boundList, selModel;
	...
	/**
	 * Return the value(s) to be submitted for this field. The returned value
	 * depends on the {@link #delimiter} config: If it is set to a String value
	 * (like the default ',') then this will return the selected values joined
	 * by the delimiter. If it is set to <tt>null</tt> then the values will be
	...
	getRawValue : function() {
		var me = this, boundList = me.boundList;
		if (boundList) {
			me.rawValue = Ext.Array.map(boundList.getSelectionModel()
	...
	setRawValue : function(value) {
		var me = this, boundList = me.boundList, models;
		...
		if (boundList) {
			...
			Ext.Array.forEach(value, function(val) {
						var undef, model = me.store.findRecord(me.valueField,
								val, undef, undef, true, true);
						if (model) {
							models.push(model);
						}
					});
	...
	getErrors : function(value) {
		var me = this, format = Ext.String.format, errors = me
				.callParent(arguments), numSelected;
	...
	/**
	 * @private Lock or unlock the BoundList's selection model to match the
	 *          current disabled/readonly state
	 */
	updateReadOnly : function() {
		var me = this, boundList = me.boundList, readOnly = me.readOnly
				|| me.disabled;
		if (boundList) {
