 ...
 * @extends Ext.form.field.Base A control that allows selection of between two
...
Ext.define('Ext.ux.form.ItemSelector', {
	...
	requires : ['Ext.ux.layout.component.form.ItemSelector',
			'Ext.button.Button'],
	...
	/**
	 * @cfg {Array} multiselects An optional array of
	 *      {@link Ext.ux.form.MultiSelect} config objects, containing
	 *      additional configuration to be applied to the internal MultiSelect
	 *      fields.
	 */
	multiselects : [],
	...
	bindStore : function(store, initial) {
		var me = this, toField = me.toField, fromField = me.fromField, models;
	...
	onRender : function(ct, position) {
		var me = this, baseCSSPrefix = Ext.baseCSSPrefix, ddGroup = 'ItemSelectorDD-'
				+ Ext.id(), commonConfig = {
			...
			hideLabel : true
				...
				}, me.multiselects[0], commonConfig), toConfig = Ext.apply({
					listTitle : 'Selected',
					store : Ext.create('Ext.data.Store', {
								model : me.store.model
							}), // blank store to begin
				...
				}, me.multiselects[1], commonConfig), fromField = Ext.widget(
				'multiselect', fromConfig), toField = Ext.widget('multiselect',
				toConfig), innerCt, buttons = [];
	...
	getSelections : function(list) {
		var store = list.getStore(), selections = list.getSelectionModel()
				.getSelection(), i = 0, len = selections.length;
	...
	onTopBtnClick : function() {
		var list = this.toField.boundList, store = list.getStore(), selected = this
				.getSelections(list), i = selected.length - 1, selection;
	...
	onBottomBtnClick : function() {
		var list = this.toField.boundList, store = list.getStore(), selected = this
				.getSelections(list), i = 0, len = selected.length, selection;
	...
	onUpBtnClick : function() {
		var list = this.toField.boundList, store = list.getStore(), selected = this
				.getSelections(list), i = 0, len = selected.length, selection, index;
	...
	onDownBtnClick : function() {
		var list = this.toField.boundList, store = list.getStore(), selected = this
				.getSelections(list), i = 0, len = selected.length, max = store
				.getCount(), selection, index;
	...
	setRawValue : function(value) {
		var me = this, Array = Ext.Array, toStore, fromStore, models;
		...
		if (me.toField) {
			...
			Ext.Array.forEach(value, function(val) {
						var undef, model = fromStore.findRecord(me.valueField,
	...
	getRawValue : function() {
		var me = this, toField = me.toField, rawValue = me.rawValue;

		if (toField) {
			rawValue = Ext.Array.map(toField.boundList.getStore().getRange(),
					function(model) {
	...
	updateReadOnly : function() {
		var me = this, readOnly = me.readOnly || me.disabled;
