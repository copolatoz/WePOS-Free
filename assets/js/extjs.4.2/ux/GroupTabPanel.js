 ...
 * @extends Ext.Container A TabPanel with grouping support.
 */
Ext.define('Ext.ux.GroupTabPanel', {
	...
	requires : ['Ext.data.*', 'Ext.tree.*', 'Ext.layout.*'],
	...
	createItemsStore : function() {
		...
		Ext.each(me.items, function(item, idx) {
			var items = item.items, rootItem = (items[item.mainItem] || items[0]), root = {
				children : []
			};
