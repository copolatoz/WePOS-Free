...
Ext.define('Ext.ux.GMapPanel', {
	...
	afterRender : function() {
		...
		if (typeof this.setCenter === 'object') {
			if (typeof this.setCenter.geoCodeAddr === 'string') {
				...
				if (typeof this.setCenter.marker === 'object'
						&& typeof point === 'object') {
					this.addMarker(point, this.setCenter.marker,
	...
	addMarkers : function(markers) {
		...
		if (Ext.isArray(markers)) {
			for (var i = 0; i < markers.length; i++) {
				...
				this.addMarker(mkr_point, markers[i].marker, false,
						markers[i].setCenter, markers[i].listeners);
			}
		}
	...
	geoCodeLookup : function(addr) {
		...
		this.geocoder.getLocations(addr, Ext.Function.bind(
						this.addAddressToMap, this));

	},
	addAddressToMap : function(response) {
		var place, addressinfo, accuracy, point;
		if (!response || response.Status.code != 200) {
			Ext.MessageBox.alert('Error', 'Code ' + response.Status.code
							+ ' Error Returned');
		} else {
			...
			if (accuracy === 0) {
				Ext.MessageBox.alert('Unable to Locate Address',
				...
				if (accuracy < 7) {
					...
					if (typeof this.setCenter.marker === 'object'
							&& typeof point === 'object') {
						this.addMarker(point, this.setCenter.marker,
								this.setCenter.marker.clear, true,
