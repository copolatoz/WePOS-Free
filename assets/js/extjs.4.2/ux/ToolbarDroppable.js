 ...
 * @extends Object Plugin which allows items to be dropped onto a toolbar and be
 *          turned into new Toolbar items. To use the plugin, you just need to
 *          provide a createItem implementation that takes the drop data as an
 *          argument and returns an object that can be placed onto the toolbar.
 *          Example:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * The afterLayout function can also be overridden, and is called after
 *          a new item has been created and inserted into the Toolbar. Use this
 *          for any logic that needs to be run after the item has been created.
 */
Ext.define('Ext.ux.ToolbarDroppable', {
	...
	calculateEntryIndex : function(e) {
		var entryIndex = 0, toolbar = this.toolbar, items = toolbar.items.items, count = items.length, xTotal = toolbar
				.getEl().getXY()[0], xHover = e.getXY()[0] - xTotal;

		for (var index = 0; index < count; index++) {
			var item = items[index], width = item.getEl().getWidth(), midpoint = xTotal
					+ width / 2;
