 ...
 * @extends Ext.grid.Panel A Grid which creates itself from an existing HTML
 *          table element.
 ...
 *            table The table element from which this grid will be created - The
 *            table MUST have some type of size defined for the grid to fill.
 *            The container will be automatically set to position relative if it
 *            isn't already.
 * @param {Object}
 *            config A config object that sets properties on this grid and has
 *            two additional (optional) properties: fields and columns which
...
Ext.define('Ext.ux.grid.TransformGrid', {
	...
	constructor : function(table, config) {
		...
		var configFields = config.fields || [], configColumns = config.columns
				|| [], fields = [], cols = [], ct = table.insertSibling(), headers = table
