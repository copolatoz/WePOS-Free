...
Ext.define('Ext.ux.BoxReorderer', {
	 ...
	 *      <p>
	 *      Optional. Defaults to <code>'.x-box-item'</code>
	 *      <p>
	 *      A {@link Ext.DomQuery DomQuery} selector which identifies the
	 *      encapsulating elements of child Components which participate in
	 *      reordering.
	 *      </p>
	 */
	 ...
	 *      </p>
	 *      <p>
	 *      If truthy, child reordering is animated so that moved boxes slide
	 *      smoothly into position. If this option is numeric, it is used as the
	 *      animation duration <b>in milliseconds</b>.
	 *      </p>
	 */
	animate : 100,
	...
	init : function(container) {
		...
		this.container.afterLayout = Ext.Function.createSequence(
				this.container.afterLayout, this.afterFirstLayout, this);

		container.destroy = Ext.Function.createSequence(container.destroy,
				this.onContainerDestroy, this);
	},
	...
	afterFirstLayout : function() {
		var me = this, l = me.container.getLayout();
		...
		// class.
		me.dd = Ext.create('Ext.dd.DD', l.innerCt, me.container.id
						+ '-reorderer');
		Ext.apply(me.dd, {
					animate : me.animate,
					reorderer : me,
					...
					clickValidator : Ext.Function.createInterceptor(
							me.dd.clickValidator, me.clickValidator, me, false),
	...
	startDrag : function() {
		...
		if (me.dragCmp) {
			...
			// positioning of the dragged *Component*
			me.dragCmp.setPosition = Ext.emptyFn;

			// If the BoxLayout is not animated, animate it just for the
			...
			me.reorderer.fireEvent('StartDrag', me, me.container, me.dragCmp,
					me.curIndex);
	...
	doSwap : function(newIndex) {
		var me = this, items = me.container.items, orig, dest, tmpIndex;
		...
		me.reorderer.fireEvent('ChangeIndex', me, me.container, me.dragCmp,
		...
		if (me.dragCmp) {
			...
			if (me.animate) {
				me.container.layout.animate = {
					...
					callback : Ext.Function.bind(me.reorderer.afterBoxReflow,
							me)
				};
			}
	...
	getNewIndex : function(pointerPos) {
		var me = this, dragEl = me.getDragEl(), dragBox = Ext.fly(dragEl)
				.getPageBox(), targetEl, targetBox, targetMidpoint, i = 0, it = me.container.items.items, ln = it.length, lastPos = me.lastPos;
