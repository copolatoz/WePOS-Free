...
Ext.define('Ext.ux.RowExpander', {
	...
	/**
	 * @cfg {Boolean} selectRowOnExpand <tt>true</tt> to select a row when
	 *      clicking on the expander icon (defaults to <tt>false</tt>).
	 */
	 ...
	 * @param {HtmlElement}
	 *            rowNode The &lt;tr> element which owns the expanded row.
	 * @param {Ext.data.Model}
	 *            record The record providing the data.
	 * @param {HtmlElement}
	 *            expandRow The &lt;tr> element containing the expanded data.
	 */
	/**
	...
	constructor : function() {
		...
		// just return it back!
		var rowBodyTpl = Ext.create('Ext.XTemplate', this.rowBodyTpl), features = [
				{
					ftype : 'rowbody',
	...
	onEnter : function(e) {
		var view = this.view, ds = view.store, sm = view.getSelectionModel(), sels = sm
				.getSelection(), ln = sels.length, i = 0, rowIdx;
	...
	toggleRow : function(rowIdx) {
		var rowNode = this.view.getNode(rowIdx), row = Ext.get(rowNode), nextBd = Ext
				.get(row).down(this.rowBodyTrSelector), record = this.view
				.getRecord(rowNode);
