...
Ext.define('Ext.ux.DataView.Animated', {
	...
	init : function(dataview) {
		...
		var idProperty = this.idProperty, store = dataview.store;
		...
		function reDraw(store) {
			...
			Ext.iterate(previous, function(id, item) {
						...
						oldPositions[id] = {
							top : el.getTop() - parentEl.getTop()
									- el.getMargin('t')
									- parentEl.getPadding('t'),
							left : el.getLeft() - parentEl.getLeft()
									- el.getMargin('l')
			...
			Ext.iterate(store.data.items, function(item) {
				var id = item.internalId, el = elCache[id];
			...
			Ext.iterate(added, function(id, item) {
						Ext.fly(this.dataviewID + '-' + item.internalId)
								.applyStyles({
									top : newPositions[item.internalId].top
											+ "px",
									left : newPositions[item.internalId].left
											+ "px"
								}).setDisplayed(true);
