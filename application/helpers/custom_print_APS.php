...
<body>
<div class="report_area" style="font-family: UbuntuMono; padding:0px; margin:0px auto; text-align:left; border:0px solid #ccc; width:<?php echo $set_width[$printer_pin].'px'; ?>;">
	<?php
		...
		if(!empty($data_line)){
			...
			foreach($xplode_perline as $key => $dt_exp){
				...
				if(in_array($dt_exp, $size_text)){
					...
					if($dt_exp == '[size=1'){
						$font_start_html .= 'font-size:13px;';
					}
					if($dt_exp == '[size=2'){
						$font_start_html .= 'font-size:16px;';
					}
					if($dt_exp == '[size=3'){
						$font_start_html .= 'font-size:18px;';
					}
