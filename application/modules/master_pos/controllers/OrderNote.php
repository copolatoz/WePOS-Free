...
class OrderNote extends MY_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('model_ordernote', 'm');
	}
	...
	public function gridData()
	{
		...
		if(!empty($is_dropdown)){
			$params['order'] = array('order_note_text' => 'ASC');
			//$params['where'] = array('parent_id != 0');
		}
		if(!empty($searching)){
			$params['where'][] = "(order_note_text LIKE '%".$searching."%')";
		}
		...
		if(!empty($show_all_text)){
			$dt = array('id' => '-1', 'order_note_text' => 'Pilih Semua');
			array_push($newData, $dt);
		}else{
			if(!empty($show_choose_text)){
				$dt = array('id' => '', 'order_note_text' => 'Pilih');
				array_push($newData, $dt);
			}
		}
	...
	public function save()
	{
		...
		$order_note_text = $this->input->post('order_note_text');
		...
		$r = '';
		if($this->input->post('form_type_orderNote', true) == 'add')
		{
			$var = array(
				'fields'	=>	array(
		...
		if($this->input->post('form_type_orderNote', true) == 'edit'){
