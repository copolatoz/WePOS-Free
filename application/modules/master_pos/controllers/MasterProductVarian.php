		...
		if($this->input->post('form_type_masterProductVarian', true) == 'add' AND $active_old_data == false)
		{
			...
			{ 
				...
				if($get_gramasi->num_rows() > 0){
					...
					$this->db->update($this->table_product, $update_gramasi, "id = ".$product_id." AND (varian_id = 0 OR varian_id IS NULL) ");
				}
        ...
        {  
			...
			if($dt_varian->num_rows() > 0){
				...
				if($has_active_varian->num_rows() > 0){
					...
					$update_gramasi = array('varian_id' => 0);
					$this->db->update($this->table_product_gramasi, $update_gramasi, "product_id = ".$product_id);
