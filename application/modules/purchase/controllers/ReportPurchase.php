...
class ReportPurchase extends MY_Controller {
	...
	public function print_reportPurchase(){
			...
			if(!empty($all_po_id)){
				...
				if($get_det->num_rows() > 0){										
					foreach($get_det->result_array() as $dt){
						$newData[$dt['po_id']]['total_qty'] += $dt['total_qty'];
		...
		$useview = 'print_reportPurchase';
		if($do == 'excel'){
			$useview = 'excel_reportPurchase';
		}
