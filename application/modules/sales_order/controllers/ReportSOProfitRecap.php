...
class ReportSOProfitRecap extends MY_Controller {
	...
	public function print_reportSOProfitRecap(){
		...
		$data_post = array(
			...
			'report_name'	=> 'SALES ORDER PROFIT REPORT (RECAP)',
			'date_from'	=> $date_from,
			'date_till'	=> $date_till,
				...
				if($get_det->num_rows() > 0){										
					foreach($get_det->result_array() as $dt){
						if(!empty($all_so_id_date[$dt['so_id']])){
							...
							$newData[$getDate]['total_hpp'] += ($dt['item_hpp'] * $dt['total_qty']);
							$newData[$getDate]['total_sales'] += ($dt['sod_total']);
							$newData[$getDate]['total_potongan'] += ($dt['sod_potongan']);
							$newData[$getDate]['subtotal'] += ($dt['sod_total'] - $dt['sod_potongan']);							
		...
		$useview = 'print_reportSOProfitRecap';
		if($do == 'excel'){
			$useview = 'excel_reportSOProfitRecap';
		}
