...
class BillingCashier extends MY_Controller {
	...
	public function getBilling($billing_id = ''){
		...
		$this->db->select('a.id, a.table_id, a.table_no, a.billing_no, a.payment_date,
			...
			a.sales_id, a.sales_percentage, a.sales_price, a.sales_type,  
			a.id as billing_id, a.voucher_no, a.is_sistem_tawar, 
			...
			g.billing_no as merge_billing_no, h.sales_name, h.sales_company');
		$this->db->from($this->table." as a");
		...
		if($get_last->num_rows() > 0){
			...
			//sales
			$billingData->sales_name_company_fee = '-- NO SALES --';
			if(!empty($billingData->sales_id)){
				...
				$billingData->sales_name_company_fee = $billingData->sales_name.' / '.$billingData->sales_company.' ('.$sales_type_simple.' '.$jenis_fee.')';
			}
		}
		...
		if($form_type_orderProduct == 'add')
		{
			...
			{  
				if(!empty($billingData->id)){
					//$update_billing = $this->calculateBilling($billingData->id);
					...
					if(!empty($update_billing)){
						...
						$billingData->total_billing = $update_billing['total_billing'];
						$billingData->tax_total = $update_billing['tax_total'];
						$billingData->service_total = $update_billing['service_total'];
						$billingData->discount_total = $update_billing['discount_total'];
						$billingData->grand_total = $update_billing['grand_total'];
						$billingData->total_pembulatan = $update_billing['total_pembulatan'];
						$billingData->total_dp = $update_billing['total_dp'];
						$billingData->compliment_total = $update_billing['compliment_total'];
						$billingData->compliment_total_tax_service = $update_billing['compliment_total_tax_service'];
						$billingData->total_billing_display = $update_billing['total_billing_display'];
						
						$billingData->total_billing_show =  priceFormat($billingData->total_billing);
						$billingData->tax_total_show =  priceFormat($billingData->tax_total);
						$billingData->service_total_show =  priceFormat($billingData->service_total);
						$billingData->discount_total_show =  priceFormat($billingData->discount_total);
						$billingData->grand_total_show =  priceFormat($billingData->grand_total);
						$billingData->total_pembulatan_show =  priceFormat($billingData->total_pembulatan);
						$billingData->total_dp_show =  priceFormat($billingData->total_dp);
						$billingData->compliment_total_show =  priceFormat($billingData->compliment_total);
						$billingData->compliment_total_tax_service_show =  priceFormat($billingData->compliment_total_tax_service);
						
						/*
						$s = (array) $billingData;
						
						if(empty($s['total_billing'])){
							$s['total_billing'] = 0;
						}
						if(empty($s['total_paid'])){
							$s['total_paid'] = 0;
						}
						...
						$s['pembulatan_dinamis'] = $get_opt['pembulatan_dinamis'];
						
						$billingData = (object) $s;
						*/
					}
	...
	public function calculateBilling($billing_id = ''){
		...
		if(!empty($billing_id)){
			...
			$get_opt_var = array('cashier_max_pembulatan','cashier_pembulatan_keatas','pembulatan_dinamis','diskon_sebelum_pajak_service');
			$get_opt = get_option_value($get_opt_var);
				...
				foreach($get_detail->result() as $dt){
					...
					if(!empty($include_tax) OR !empty($include_service)){
						if(!empty($include_tax) AND !empty($include_service)){
							...
							if($diskon_sebelum_pajak_service == 1 AND !empty($discount_price)){
			...
			if($total_pembulatan == 100 OR $total_pembulatan == 0){
				$total_pembulatan = 0;
			}
				
			$grand_total_all += $total_pembulatan;
	...
	public function doPrint($is_void = '', $void_id = 0, $order_detail_id = ''){
		...
		if(($tipe == 'payBilling' AND !empty($id)) OR (!empty($is_void) AND !empty($void_id))){
			...
			if(!empty($billingData)){
				...
				$this->db->select("a.*, d.table_no, a2.billing_no,
				...
				if($get_detail->num_rows() > 0){
					...
					foreach($get_detail->result() as $bil_det){
						...
						$product_order_qty = $bil_det->order_qty;
						
						$order_data .= "[align=0]".$product_order_qty."[tab]".$product_name."[tab]".$product_price_show."[tab]".$order_total_show;
						$order_data2 .= "[align=0]".$product_order_qty."[tab]".$product_name."[tab] [tab]".$order_total_show;
						...
						if($bil_det->product_group == 'food'){
							...
							if($is_void_order){
								...
								$order_data_kitchen[$bil_det->id] .= $product_order_qty."[tab]".$bil_det->product_name.$product_chinese_name.$varian_name.$takeaway_name.$order_notes."\n";
						...
						if($bil_det->product_group == 'beverage'){
							...
							if($is_void_order){
								...
								$order_data_bar[$bil_det->id] .= $product_order_qty."[tab]".$bil_det->product_name.$product_chinese_name.$varian_name.$takeaway_name.$order_notes."\n";
							...
							if($is_void_order){
								...
								$order_data_other[$bil_det->id] .= $product_order_qty."[tab]".$bil_det->product_name.$product_chinese_name.$varian_name.$takeaway_name.$order_notes."\n";
						...
						if(!empty($diskon_name)){
							...
							if($bil_det->is_promo == 1 AND !empty($bil_det->promo_id)){
								...
								$order_data .= "\n"."[align=0] [tab]".$diskon_name."[tab] [tab]";
								$order_data2 .= "\n"."[align=0] [tab]".$diskon_name."[tab] [tab]";
									...
									if(!empty($allow_diskon_product)){
										if(in_array($dt->product_id, $allow_diskon_product)){
											...
											if(!empty($data_diskon->discount_percentage)){
												...
												$product_price_discount = priceFormat(($data_diskon->discount_percentage / 100) * $product_price_real, 0, ".", "");
											...
											if(!empty($data_diskon->discount_price)){
												...
												$product_price_discount = priceFormat($product_price_real - $data_diskon->discount_price, 0, ".", "");
												$s['discount_price'] = $product_price_discount;
												$s['discount_total'] = $product_price_discount * $order_qty;
												//$s['status_discount'] = 1;
											}
