...
class KontrabonReport extends MY_Controller {
	...
	public function print_kontrabonReport(){
		...
		if(empty($date_from) OR empty($date_till)){
			...
			$add_where = "(a.kb_date >= '".$qdate_from."' AND a.kb_date <= '".$qdate_till."')";
			...
			if(!empty($status)){
				$this->db->where("a.kb_status = '".$status."'");
				$data_post['kb_status'] = $status;
			}
			
			$this->db->where("a.is_deleted", 0);
			$this->db->where($add_where);
			$this->db->order_by("kb_date","ASC");
			$get_dt = $this->db->get();
			...
			if(!empty($data_post['report_data'])){
				foreach ($data_post['report_data'] as $s){
					...
					$s['kb_date'] = date("d-m-Y",strtotime($s['kb_date']));
		...
		$useview = 'print_kontrabonReport';
		if($do == 'excel'){
			$useview = 'excel_kontrabonReport';
		}
