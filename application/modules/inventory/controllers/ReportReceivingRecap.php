...
class reportReceivingRecap extends MY_Controller {
	...
	public function print_reportReceivingRecap(){
		...
		$data_post = array(
			...
			'report_name'	=> 'RECEIVING REPORT (RECAP)',
			'date_from'	=> $date_from,
			'date_till'	=> $date_till,
		...
		if(empty($date_from) OR empty($date_till)){
			...
			if(!empty($data_post['report_data'])){
				foreach ($data_post['report_data'] as $s){
					...
					if(empty($newData[$s['receive_date']])){
						$newData[$s['receive_date']] = array(
							'date'			=> $s['receive_date'],
							'total_receiving'	=> 0,
							'total_item'	=> 0,
							'total_qty'		=> 0,
					...
					$newData[$s['receive_date']]['total_receiving'] += 1;
					$newData[$s['receive_date']]['discount'] += $s['discount'];
					//array_push($newData, $s);
					
					if(empty($all_receive_id_date[$s['id']])){
						$all_receive_id_date[$s['id']] = $s['receive_date'];
					}
				...
				if($get_det->num_rows() > 0){										
					foreach($get_det->result_array() as $dt){
						if(!empty($all_receive_id_date[$dt['receive_id']])){
							$getDate = $all_receive_id_date[$dt['receive_id']];
							
							$newData[$getDate]['total_qty'] += $dt['total_qty'];
							$newData[$getDate]['total_price'] += ($dt['item_price'] * $dt['total_qty']);							
		...
		$useview = 'print_reportReceivingRecap';
		if($do == 'excel'){
			$useview = 'excel_reportReceivingRecap';
		}
