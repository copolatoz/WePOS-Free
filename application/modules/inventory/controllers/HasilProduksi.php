...
class HasilProduksi extends MY_Controller {
	...
	public function save()
	{
		...
		if($is_closing){
			$r = array('success' => false, 'info' => 'Transaksi untuk Inventory pada tanggal: '.$pr_date.' sudah ditutup!'); 
			die(json_encode($r));
		}
		...
		if(!empty($pr_to)){
			$this->stock->cek_storehouse_access($pr_to);
		}
			...
			*/
			...
			{  
				...
				if($warning_update_stok){
					$r['is_warning'] = 1;
					$r['info'] = 'Silahkan Re-Generate/Perbaiki Stok Transaksi pada List Stock Module!<br/>Perbaiki Stok dari: '.$pr_date;
				}
				*/
		...
		{ 
			...
			//{
				...
				//if($do_update_stok OR $do_update_rollback_stok){
				$q_det = $this->m2->hasilProduksiDetail($hasilProduksiDetail, $id);
				
				$old_status = '';
				if(!empty($old_data['pr_status'])){
					$old_status = $old_data['pr_status'];
				}
				
				if($pr_status == 'done' AND $old_status != 'done'){
					...
					$r['hasilProduksiDetail_done'] = $hasilProduksiDetail;
				}
				
				
			//}
				
			$q_det = $this->m2->hasilProduksiDetail($hasilProduksiDetail, $id, $update_stok);
			if($q_det == false){
				$r = array('success' => false, 'info' => 'Input Detail Production Failed!'); 
				die(json_encode($r));
			}
			...
			if(!empty($q_det['dtProduction']['pr_number'])){
				$r['pr_number'] = $q_det['dtProduction']['pr_number'];
			}
			...
			//$updateAR = $this->account_receivable->set_account_receivable_Production($id);
			//$r['success'] = false;
		}  
		else
		{  
