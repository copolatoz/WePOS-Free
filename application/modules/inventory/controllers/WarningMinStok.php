...
class warningMinStok extends MY_Controller {
	...
	public function print_warningMinStok(){
		...
		$data_post = array(
			...
			'report_name'	=> 'WARNING MINIMAL STOK',
			'category'	=> $category,
		...
		if($getItem->num_rows() > 0){
			foreach($getItem->result_array() as $dtR){
				if(!in_array($dtR['id'], $all_item_id)){
					...
					if($dtR['total_stock'] <= $dtR['min_stock']){
						$show_item = true;
					}else{
						$show_item = false;
					}
		...
		$useview = 'print_warningMinStok';
		if($do == 'excel'){
			$useview = 'excel_warningMinStok';
		}
