...
class reportReceivingDetailPerItem extends MY_Controller {
	...
	public function print_reportReceivingDetailPerItem(){
		...
		$data_post = array(
			...
			'report_name'	=> 'RECEIVING DETAIL PER-ITEM',
			'month'	=> $month,
			'year'	=> $year,
		...
		//VERSI STOCK TRX
		$available_cat_trx = array();
		$available_stok_trx = array();
		$available_category_trx = array();
		...
		if($get_trx->num_rows() > 0){
			foreach($get_trx->result_array() as $dtR){
				...
				if(!empty($all_item[$dtR['item_id']])){
					...
					//if($dtR['trx_ref_data'] == 'RL1608087'){ echo $dtR['trx_date'].' -> '.$get_day.', '.$dtR['item_id'].' == '.$dtR['trx_qty'].'<br/>';}	
					if($dtR['trx_type'] == 'in'){
					...
					if(!in_array($all_item[$dtR['item_id']]['category_id'], $available_cat_trx)){
						$available_cat_trx[] = $all_item[$dtR['item_id']]['category_id'];
					}
		...
		$allCat_new = array();
		if(!empty($allCat)){
			foreach($allCat as $catId => $dt){
				if(in_array($catId, $available_cat_trx)){
					$allCat_new[$catId] = $dt;
				}
			}
		...
		$allCat_item_new = array();
		if(!empty($allCat_item)){
			foreach($allCat_item as $catId => $dtItem){
				
				if(!empty($allCat[$catId])){
					
					if(!empty($dtItem)){
						foreach($dtItem as $itemId){
							if(in_array($itemId, $available_stok_trx)){
						
								if(empty($allCat_item_new[$catId])){
									$allCat_item_new[$catId] = array();
								}
								
								if(!in_array($itemId, $allCat_item_new[$catId])){
									$allCat_item_new[$catId][] = $itemId;
								}
			...
			$allCat_item = $allCat_item_new;
		}
		
		/*echo '<pre>';
		print_r($available_stok_trx);
		print_r($available_cat_trx);
		print_r($allCat);
		print_r($allCat_item);
		die();
		*/
		...
		$data_post['available_stok_trx'] = $available_stok_trx;
		$data_post['available_cat_trx'] = $available_cat_trx;
		...
		$useview = 'print_reportReceivingDetailPerItem';
		if($do == 'excel'){
			$useview = 'excel_reportReceivingDetailPerItem';
		}
