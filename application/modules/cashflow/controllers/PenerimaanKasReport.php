...
class PenerimaanKasReport extends MY_Controller {
	...
	public function print_penerimaanKasReport(){
		
		$this->table = $this->prefix_acc.'penerimaan_kas';
		...
		$data_post = array(
			...
			'report_name'	=> 'PENERIMAAN KAS REPORT',
			'date_from'	=> $date_from,
			'date_till'	=> $date_till,
		...
		if(empty($date_from) OR empty($date_till)){
			...
			$add_where = "(a.km_date >= '".$qdate_from."' AND a.km_date <= '".$qdate_till."')";
			...
			$this->db->join($this->prefix_acc.'tujuan_cashflow as c','c.id = a.km_tujuan','LEFT');
			...
			if(!empty($data_post['report_data'])){
				foreach ($data_post['report_data'] as $s){
					...
					if(!in_array($s['id'], $all_km_id)){
						$all_km_id[] = $s['id'];
					}		
										
					$s['km_total_text'] = priceFormat($s['km_total']);
		...
		$useview = 'print_penerimaanKasReport';
		if($do == 'excel'){
			$useview = 'excel_penerimaanKasReport';
		}
