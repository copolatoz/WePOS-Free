...
class reportSalesCustomer extends MY_Controller {
	...
	public function print_reportSalesCustomer(){
		...
		$data_post = array(
			...
			'report_name'	=> 'SALES CUSTOMER REPORT',
			'date_from'	=> $date_from,
			'date_till'	=> $date_till,
		...
		if(empty($date_from) OR empty($date_till)){
			...
			$this->db->select("a.*, a.id as billing_id, a.updated as billing_date, d.payment_type_name, e.bank_name, f.customer_name, f.customer_contact_person");
			...
			$this->db->join($this->prefix.'customer as f','f.id = a.customer_id','LEFT');
		...
		$useview = 'print_reportSalesCustomer';
		$data_post['report_name'] = 'SALES CUSTOMER REPORT';
		$data_post['customer_name_report'] = $customer_name_report;
		
		if($do == 'excel'){
			$useview = 'excel_reportSalesCustomer';
		}
		
		$this->load->view('../../billing/views/'.$useview, $data_post);	
	}
	
	public function print_reportSalesCustomerRecap(){
			...
			if(!empty($data_post['report_data'])){
				foreach ($data_post['report_data'] as $s){
					...
					if(!empty($s['customer_id'])){
						...
						$use_customer_total = $s['total_billing'];
					...
					$group_id = $s['customer_id'].'_'.$payment_date;
		...
		$useview = 'print_reportSalesCustomerRecap';
		$data_post['report_name'] = 'SALES CUSTOMER REPORT (RECAP)';
		$data_post['customer_name_report'] = $customer_name_report;
		
		if($do == 'excel'){
			$useview = 'excel_reportSalesCustomerRecap';
		}
