	...
	<table width="<?php echo $set_width; ?>">
		...
		<thead>
			...
			<tr>
				...
				<td class="tbl_head_td_xcenter" width="80" rowspan="2">FOOD</td>
				<td class="tbl_head_td_xcenter" width="80" rowspan="2">BEVERAGE</td>
				<td class="tbl_head_td_xcenter" width="80" rowspan="2">OTHER</td>
				<td class="tbl_head_td_xcenter" width="110" rowspan="2">TOTAL BILLING</td>
				<?php
				if($diskon_sebelum_pajak_service == 1){
					?>
					<td class="tbl_head_td_xcenter" width="100" colspan="2">DISCOUNT</td>
					<?php
				}
				?>
		...
		if(!empty($report_data)){
			...
			foreach($report_data as $det){
				...
				<tr>
					...
					<td class="tbl_data_td_xright">Rp. <?php echo $det['total_food_show']; ?></td>
					<td class="tbl_data_td_xright">Rp. <?php echo $det['total_beverage_show']; ?></td>
					<td class="tbl_data_td_xright">Rp. <?php echo $det['total_other_show']; ?></td>
			...
			<tr>
				...
				<td class="tbl_summary_td_xright">Rp. <?php echo priceFormat($total_food); ?></td>
				<td class="tbl_summary_td_xright">Rp. <?php echo priceFormat($total_beverage); ?></td>
				<td class="tbl_summary_td_xright">Rp. <?php echo priceFormat($total_other); ?></td>
				<td class="tbl_summary_td_xright">Rp. <?php echo priceFormat($total_billing); ?></td>
				<?php
				if($diskon_sebelum_pajak_service == 1){
					?>
					<td class="tbl_summary_td_xright"><?php echo priceFormat($discount_total); ?></td>
				...
				if(!empty($payment_data)){
					foreach($payment_data as $key_id => $dtPay){
						...
						$total_payment = 0;
						if(!empty($grand_total_payment[$key_id])){
							$total_payment = $grand_total_payment[$key_id];
						}
