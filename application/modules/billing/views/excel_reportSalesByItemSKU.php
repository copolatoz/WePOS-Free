	...
	<table width="<?php echo $set_width; ?>">
		...
		<thead>
			...
			<tr>
				...
				<td class="tbl_head_td_xcenter" width="110" rowspan="2">ITEM CODE</td>
					...
					if(!empty($groupCat)){
						if($groupCat == 'sku'){
							if(!empty($item_sku_name)){
								$category_name = $item_sku_name;
							}
						}
					}
					
					if(empty($category_name[$key])){
						$category_name_show = 'Unknown Category';
					}else{
						$category_name_show = $category_name[$key];
					}
					
					if($category_name_show != 'Unknown Category'){
						$category_name_show = $key.' / '.$category_name_show;
					}
